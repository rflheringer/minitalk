...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/01 10:54:56 by rheringe          #+#    #+#             */
/*   Updated: 2024/11/01 17:00:26 by rheringe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	ft_decimal(unsigned int arg, t_flags *flags)
{
	...
	{
		j = 0;
		if (flags->plus == 1 && convert_to_alpha[j] != '-')
			ft_putchar('+');
		if (flags->plus == 1 && convert_to_alpha[j] != '-')
			len = 1;
		if (flags->space == 1 && flags->plus == 0 && convert_to_alpha[j] != '-')
			ft_putchar(' ');
		if (flags->space == 1 && flags->plus == 0 && convert_to_alpha[j] != '-')
			len = 1;
	}
